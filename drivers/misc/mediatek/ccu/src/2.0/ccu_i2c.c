...
static int ccu_i2c_controller_uninit(
	enum CCU_I2C_CHANNEL i2c_controller_id);
...
int ccu_get_i2c_dma_buf_addr(struct ccu_i2c_buf_mva_ioarg *ioarg)
{
	...
	ret = i2c_query_dma_buffer_addr(ioarg->i2c_controller_id, &va,
		&ioarg->va_h, &ioarg->va_l, &ioarg->i2c_id);
	...
	int enable)
{
	...
	LOG_INF_MUST("%s, id(%d):(%d)->(%d)\n",
		 ...
		 ccu_i2c_initialized[i2c_controller_id], enable);
	...
	if (enable) {
		if (ccu_i2c_initialized[i2c_controller_id] == MFALSE) {
			ret =
			i2c_ccu_enable(pClient->adapter, I2C_BASE_OFS_CH1);
